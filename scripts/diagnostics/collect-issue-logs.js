...
function collectConfigStatus() {
  ...
  if (fs.existsSync(bridgeEnv)) {
    ...
    checks.codexCredentials =
      /^(CODEX_API_KEY|CODEX_ACCESS_TOKEN)=/m.test(envText)
        ? "✅ Found"
        : "⚠️ Not found"
  } else {
...
function collectLogs() {
  ...
  // Log files to check (auto-generated by server + legacy manual tee)
  const logFiles = [
    {
      name: "protocol-bridge.log",
      path: path.join(LOG_DIR, "protocol-bridge.log"),
    },
    { name: "cursor_grpc.log", path: path.join(LOG_DIR, "cursor_grpc.log") },
